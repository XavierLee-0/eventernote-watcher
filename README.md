...
数据（出演者、活动快照、设置）都存在 `./data/eventernote.db`，随容器升级保留；备份这个文件即可备份全部数据。镜像默认时区为北京时间（`Asia/Shanghai`），需要其他时区时在部署环境设置 `TZ` 环境变量（IANA 时区名，如 `Asia/Tokyo`）。
...
> 「设置」页底部有**配置导出/导入**：导出的 JSON 包含出演者列表、全部设置（含通知渠道 token）和活动快照，用于备份或迁移部署。含快照的导入可无缝衔接——不重建基线、迁移期间不漏报新活动；旧版无快照的文件导入后首次抓取重建基线（不触发推送轰炸）。**导出文件含密钥，请妥善保管。**
